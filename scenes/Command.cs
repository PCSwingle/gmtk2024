...
public partial class Command : Node2D {
    ...
    private const float ZoomInSpeed = 4f;
    ...
    private void _ClampPosition() {
        ...
        min += 500;
        max += 500;
        this._zoomNode.Position = this._zoomNode.Position.Clamp(new Vector2(min, min), new Vector2(max, max));
    }
    ...
    public override async void _Input(InputEvent e) {
        if (e is InputEventMouseButton { Pressed: true } me) {
            ...
            await this.ToSignal(this.GetTree().CreateTimer(0.06f), SceneTreeTimer.SignalName.Timeout);
    ...
    private static float Logerp(float a, float b, float t, bool inverse) {
        if (inverse) {
            return b * Mathf.Pow(a / b, 1 - t);
        } else {
            return a * Mathf.Pow(b / a, t);
        }
    }

    private void _scale(float to, bool up, double delta) {
        var previousZoom = this._zoom;
        this._zoom = Logerp(this._zoom, to + (up ? 0.05f : -0.05f),
            (up ? ZoomInSpeed : ZoomOutSpeed) * (float) delta, !up);
        this._zoom = up ? Mathf.Min(this._zoom, to) : Mathf.Max(this._zoom, to);
        ...
        this._zoomNode.Transform = this._zoomNode.Transform.Translated(-mouse)
            .Scaled(new Vector2(zoomRatio, zoomRatio)).Translated(mouse);
        this._ClampPosition();
    }

    public override void _PhysicsProcess(double delta) {
        if (this._zoomIn) {
            this._scale(MinExtraZoom, false, delta);
        } else if (this._zoomOut) {
            this._scale(MaxExtraZoom, true, delta);
        } else if (this._zoom is > MaxZoom) {
            this._scale(MaxZoom, false, delta);
        } else if (this._zoom < MinZoom) {
            this._scale(MinZoom, true, delta);
        }
    }
}