...
public partial class VertexIO : Node2D {
    ...
    private static readonly PackedScene VertexIoScene =
        ResourceLoader.Load<PackedScene>("res://scenes/vertex/vertex_io.tscn");

    private readonly List<int> _movedWindow = [];
    ...
    public static VertexIO CreateVertexIo(
        Vertex parent,
        string defaultLabel,
        bool isInput
    ) {
        var vertexIo = VertexIoScene.Instantiate<VertexIO>();
        vertexIo._parent = parent;
        vertexIo.Position = new Vector2(
            isInput ? 0 : 200,
            0
        );
        vertexIo.IsInput = isInput;
        vertexIo._defaultLabel = defaultLabel;
        vertexIo.Reset();
        return vertexIo;
    }
    ...
    public void Reset() {
        ...
        if (Resources.RegisteredResources.TryGetValue(
            ...
            label.Text = this._defaultLabel;
            var labelSize = label.GetMinimumSize().X;
            label.Position = new Vector2(
                this.IsInput ? Gap : -(Gap - 2) - labelSize,
                -16
            );
        }
    ...
    public void SetResource(
        ...
        var labelSize = label.GetMinimumSize().X;
        label.Position = new Vector2(
            this.IsInput ? 2 * Gap + 16 : -(Gap - 1) - labelSize,
            -16
        );
        ...
        resourceSprite.Position = new Vector2(
            this.IsInput ? Gap + 8 : -8 - (Gap * 2 - 1) - labelSize,
            0
        );
        ...
        if (withStorage) {
            ...
            storageSprite.Position = new Vector2(
                this.IsInput ? 3 * Gap - 1 + 16 + labelSize : -7 - 16 - (Gap * 3 - 1) - labelSize,
                -8
            );
            ;
        } else {
    ...
    public void SendResources() {
        if (!this.IsInput && this.Connected != null) {
            ...
            if (other.IsInput && this.ConnectedRes != null) {
                ...
                this._movedWindow.Add(moved);
                if (this._movedWindow.Count > AverageWindowLength) {
                    this._movedWindow.RemoveAt(0);
                }

                this.Connected.Line.SetStatus(
                    VertexLine.Status.Moving,
                    (float) this._movedWindow.Average() / TransferRate
                );
            }
    ...
    private void _ModifyFillLevel() {
        ...
        var storageSprite = this.GetNode<Sprite2D>("VertexIoStorageSprite");
        storageSprite.Position = new Vector2(
            storageSprite.Position.X,
            -8 + pixelsMissing
        );
        storageSprite.RegionRect = new Rect2(
            new Vector2(
                storageSprite.RegionRect.Position.X,
                pixelsMissing
            ),
            new Vector2(
                storageSprite.RegionRect.Size.X,
                16 - pixelsMissing
            )
        );

        var color = Palette.FillGradient(1 - (float) pixelsMissing / 16);
    ...
    public override void _UnhandledInput(InputEvent e) {
        if (this._dragLine != null && e is InputEventMouseButton { Pressed: false, ButtonIndex: MouseButton.Left }) {
            if (_hoveredVertexIo != null && _hoveredVertexIo._parent != this._parent) {
                this.Connected?.Delete();
                _hoveredVertexIo.Connected?.Delete();
                this.Connected = new Connection(
                    this,
                    _hoveredVertexIo
                );
            }
        ...
        if (this._dragLine != null && e is InputEventMouseMotion me) {
            this._dragLine.Points = VertexLine.GetDragPoints(
                this,
                this.ToLocal(me.Position)
            );
    ...
    private void _GuiInput(InputEvent e) {
        if (e is InputEventMouseButton { Pressed: true, ButtonIndex: MouseButton.Left } me &&
            ...
            this._dragLine = VertexLine.CreateDragLine(
                this,
                me.Position
            );
    ...
    public class Connection {
        ...
        public Connection(
            ...
            this.Line = VertexLine.CreateConnectionLine(
                this._c1,
                this._c2
            );
        ...
        private void Dirtyify() {
            if (this._c1.IsInput) {
                this._c1._parent.MarkDirty();
            }

            if (this._c2.IsInput) {
                this._c2._parent.MarkDirty();
            }
        }
        ...
        public void OnMove() {
            this.Line.Points = VertexLine.GetConnectionPoints(
                this._c1,
                this._c2
            );
